...
- Tenants & leases: `GET /api/v1/tenants` and `GET /api/v1/leases` support optional `q`, `page`, `pageSize` (leases also `tenantId`); when any of those are set, JSON is `{ items, total, page, pageSize }` — omit them for a full array (e.g. dropdowns). Writes: **SUPER_ADMIN**, **ORG_ADMIN**, **PORTFOLIO_MANAGER**. Overlapping **active** leases on the same unit are rejected (draft/terminated leases do not block).
